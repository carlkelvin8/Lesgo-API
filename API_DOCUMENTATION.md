...
**Response includes:** inline address fields + items embedded in each order.

---

### POST /orders
Create a new order. Address is passed **inline** — no need for a saved address ID.
  ...
  "items": [
    {
      ...
      "image_url": "https://cdn.example.com/items/chickenjoy.jpg",
...
|-------|----------|-------------|
| `service_id` | ? | Which service (LesGo, LesBuy, LesEat, etc.) |
| `estimated_distance_m` | ? | Distance in meters (use `/distance/calculate`) |
| `pickup.address` | ? | Human-readable pickup address |
| `pickup.lat` / `pickup.lng` | ? | GPS coordinates |
| `pickup.contact_name` | ? | Defaults to logged-in user's name |
| `pickup.contact_phone` | ? | Defaults to logged-in user's phone |
| `dropoff.*` | ? | Same as pickup fields |
| `payment_method` | ? | `cash` \| `gcash` \| `maya` \| `card` \| `wallet` (default: `cash`) |
| `notes` | ? | Order-level instructions for driver |
| `save_addresses` | ? | If `true`, saves pickup/dropoff to user's address book |
| `items` | ? | Required for LesBuy/LesEat orders |
| `items.*.name` | ? (if items) | Item name |
| `items.*.quantity` | ? (if items) | How many |
| `items.*.unit` | ? | `pcs`, `kg`, `box`, `cup`, etc. |
| `items.*.notes` | ? | Special instructions for this item |
| `items.*.image_url` | ? | Photo of the item |
| `items.*.estimated_price` | ? | Expected price per item |
| `scheduled_at` | ? | ISO datetime for scheduled orders |

**Fare Calculation (auto-computed by server):**
- Base: ?40 for first 3km
- LesGo: +?9.50/km after 3km
- Others: +?10.00/km after 3km
- LesBuy/LesEat service fee based on `meta.order_value`:
  - =?500 ? +?15 | =?1000 ? +?30 | >?1000 ? +?45
...
```json
{
  ...
  "data": {
    ...
    "estimated_fare": "86.50",
    "estimated_distance_m": 5200,
    "notes": "Please ring the doorbell",
    "pickup_address": "123 Rizal St, Manila",
    "pickup_lat": "14.5995000",
    "pickup_lng": "120.9842000",
    "pickup_contact_name": "Juan dela Cruz",
    "pickup_contact_phone": "+639171234567",
    "dropoff_address": "456 Mabini Ave, Makati",
    "dropoff_lat": "14.5547000",
    "dropoff_lng": "121.0244000",
    "dropoff_contact_name": "Maria Santos",
    "dropoff_contact_phone": "+639181234567",
    "service": { "id": 1, "name": "LesBuy", "code": "LESBUY", "icon_url": null },
    "customer": { "id": 5, "name": "Juan dela Cruz", "phone_number": "+639171234567" },
    "items": [
      {
        "id": 1,
        "name": "Jollibee Chickenjoy",
...
### GET /orders/{id}
Get full order details. Response includes all inline address fields, items, driver info, and payments.
