...
test.describe('Orders', () => {
  ...
  test('POST /orders → 201 customer creates order', async ({ request }) => {
    test.skip(!serviceId, 'No services available — seed the DB first');
    ...
    expect((body.data as any).estimated_fare).toBeGreaterThan(0);

    orderId = (body.data as any).id;
  });
  ...
  test('GET /orders → 200 paginated, customer sees own orders only', async ({ request }) => {
    ...
    items.forEach(o => expect(o.customer_id).toBeDefined());
  });
  ...
  test('GET /orders/{id} → 200 owner can view', async ({ request }) => {
    test.skip(!orderId, 'No order created yet');

    const api = new ApiClient(request);
    api.setToken(customerToken);

    const { status, body } = await api.get(`/orders/${orderId}`);

    expect(status).toBe(200);
    expect((body.data as any).id).toBe(orderId);
  });
  ...
  test('GET /orders/{id} → 403 other customer cannot view', async ({ request }) => {
    ...
    const { status } = await api.get(`/orders/${orderId}`);
  ...
  test('PATCH /orders/{id}/status → 200 customer cancels own pending order', async ({ request }) => {
    ...
    const { status, body } = await api.patch(`/orders/${orderId}/status`, {
      status:        'cancelled',
